...
def compute_path():
    try:
        ...
        path, debug_msgs = dstar.compute_path((start_lat,start_lon),(goal_lat,goal_lon),corridor_m=corridor_m,debug=True)
        if not path:
...
def tile_bounds():
    ...
    def tile2lat(y,z): n=math.pi-2*math.pi*y/(2**z); return math.degrees(math.atan(np.sinh(n)))
    west = tile2lon(min_x,zoom); east=tile2lon(max_x+1,zoom)
    north=tile2lat(min_y,zoom); south=tile2lat(max_y+1,zoom)
    center_x=(min_x+max_x)/2+0.5; center_y=(min_y+max_y)/2+0.5
    center_lon=tile2lon(center_x,zoom); center_lat=tile2lat(center_y,zoom)
